options = {                 # DEFAULTS:
  clear_database: false,    # false
  create_test_users: false, # false
  print_messages: true,     # true
  consumers: 5000,          # 5000
  providers: 5000,          # 5000
  pasadena_consumers: 0,    # 0
  jobs: 8500,               # 8500
  all_jobs_posted: false    # false
}
...
def seed_db(options)
  ...
  if options[:print_messages] && options[:consumers] > 0
    p "Fake consumers created"
  end

  options[:providers].times do
    Provider.create(
      first_name: Faker::Name.first_name,
      last_name: Faker::Name.last_name,
      email: Faker::Internet.email,
  ...
  if options[:print_messages] && options[:providers] > 0
    p "Fake providers created"
  end
  ...
  options[:pasadena_consumers].times do
    Consumer.create(
      first_name: Faker::Name.first_name,
      last_name: Faker::Name.last_name,
      email: Faker::Internet.email,
  ...
  if options[:print_messages] && options[:pasadena_consumers] > 0
    p "Pasadena consumers created"
  end

  statuses = ["posted", "claimed", "started", "completed"]
  consumer_ids = Consumer.ids
  ...
  # 10000.times do
    ...
    if options[:all_jobs_posted]
      job_instance.status = "posted"
    else
      job_instance.status = statuses.sample
    end
  ...
  p "seeds.rb has finished running. Please restore options to default settings."
end
